...
def calcAllBetas2(start_date_arr, end_date_arr, symbols_list):
    ...
    betas = ft.featBeta(d_data, len(ldt_timestamps), 'SPY')
...
def port_select(beta_list):
    ...
    for _ in xrange(0, int(210)):
        ...
        if (beta_sum >= 4.8 and beta_sum <= 6):    #change this
    	    #print cand_port
            #portfolio_list.append(cand_port)
	        return cand_port

def backtest_list(start_date_arr, end_date_arr, port_list, sharpe_threshold):
    start_date = dt.datetime(start_date_arr[0], start_date_arr[1], start_date_arr[2])
    end_date = dt.datetime(end_date_arr[0], end_date_arr[1], end_date_arr[2])  
    #fh = open("./portSharpe.csv", "w")
    results_list = []
    for port in port_list:
        symbols = [sym for sym, _ in port] #isolate symbols

        #port is [(symbol, beta), ...]
        sym_list, pBeta, sharpe = backTest.simulate(start_date, end_date, port, [0.25, 0.25, 0.25, 0.25])
        line = ",".join(symbols) + "," + str(pBeta) + "," + str(sharpe) + "\n"
        #fh.writelines(line)
        print line.replace("\n", "")
        results_list.append((symbols, pBeta, sharpe))
    #fh.close()
    return results_list
...
symbols = sample(da.DataAccess('Yahoo').get_symbols_from_list('sp5002008'), 250)    #random subset of symbols
#symbols = da.DataAccess('Yahoo').get_symbols_from_list('sp5002008')[:250]           #first 10 symbols
#symbols = da.DataAccess('Yahoo').get_symbols_from_list('sp5002008')                 #all symbols
...
#print [x for x in next(backtest_list([2009, 1, 1], [2009,12,31], port_list, 1.2))]
